...
begin
  ...
  begin
    ...
    WaitForBarrier(TestDone, 200 us) ;

    TranscriptClose ;
    -- Printing differs in different simulators due to differences in process order execution
    -- AffirmIfTranscriptsMatch(PATH_TO_VALIDATED_RESULTS) ;

    EndOfTestReports(TimeOut => (now >= 200 us)) ;
    std.env.stop ;
    wait ;
  ...
  begin
    ...
    -- Read the PCI configuration space structure and decode
    for ridx in 0 to 15 loop
      PcieCfgSpaceRead(UpstreamRec, std_logic_vector(to_unsigned((ridx*4), 16)), X"02_0_0", Data(31 downto 0), PcieStatus) ;
      AffirmIfEqual(PcieStatus.Packet, PKT_STATUS_GOOD, "Config Space Read Error Status #"      & integer'image(5 + ridx) & ": ") ;
      AffirmIfEqual(PcieStatus.Completion, CPL_SUCCESS, "Config Space Read Completion Status #" & integer'image(5 + ridx) & ": ") ;
      PcieDecodePciRegisters(Data, ridx) ;
      WaitForClock(UpstreamRec, WaitForClockRV.RandInt(1, 5)) ;
    end loop ;
